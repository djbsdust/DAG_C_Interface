...
int main(int argc,char **argv){
    ...
    //              L1                              R: X[0][0:N-1:bz]     X[i][0]
    //            /    \                          
    //           /      \                         
    //          L2       \                                                X[i][0]
	//                   L3                                               X[i][0]
    //        -----------------------  barrier
    //                L4                            R: X[0:N-1:bz][0]     X[0][j] 
    //              /     \                         
    //             /       \                          
    //            L5       L6                                             X[0][j]
    //        -----------------------  barrier
    ...
    printf("USAGE: ./a.out nter blkx blky num_threads\n");
    if(argc>1)
      T=atoi(argv[1]);
    if(argc>2)
      blockx=atoi(argv[2]);
    ...
    for (t = 0; t < T; t++) {
       ...
       for (i=0; i<N; i+=blockx) {
		 ...
		 //use X[id][0] to represent the data region X[i:i+bz-1][0:N-1]
		 //so as the data region B[i:i+bz-1][0:N-1]
...
#endif
    printf("threads= %d\n",threads);
    IF_TIME(printf("total_time %0.6lf\n", t_end - t_start));
