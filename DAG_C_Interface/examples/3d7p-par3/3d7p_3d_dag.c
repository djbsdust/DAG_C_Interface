...
void my_neighbors(struct ptr_array *neighbor_addrs, targs* args)
{
    ...
    if(startj + BLKY < SIZEY)
        neighbor_addrs->arr[(neighbor_addrs->len)++]=(void*)(src_addr + BLKY * SIZEZ* 8);
    if(startk - BLKZ >= 0)
        neighbor_addrs->arr[(neighbor_addrs->len)++]=(void*)(src_addr - BLKZ * 8);
    if(startk + BLKZ < SIZEZ)
        neighbor_addrs->arr[(neighbor_addrs->len)++]=(void*)(src_addr + BLKZ* 8);
}
			...
			, const int blocsizez)
{
    ...
    for(itr = 0; itr < iter; ++itr)
    {
        ...
        for(i = 1; i < SIZEX -1; i += blocsizex)
        {
            ...
            for(k = 1; k < SIZEZ -1; k += blocsizez)
            {
                int indexk = (k-1) / blocsizez;
                int endk = (k + blocsizez > SIZEZ -1 ) ? SIZEZ - 1 : k + blocsizez;
                each.starti=i;
                each.endi=endi;
                each.startj=j;
                each.endj=endj;
...
int main(int argc, char** argv)
{
    ...
    if(argc > 6)
    {
        printf("input arg error, USAGE: ./a.out nter blkx blky blkz num_threads\n");
        return 0;
    }
    num_threads = 1;
    BLKX = 32;
